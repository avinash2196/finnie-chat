    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for symbol in symbols:
                ...
                print(f"{symbol:8} | Price: ${price:>10,.2f} | Delta: {change:>+10,.2f} | Change: {change_pct:>+6.2f}%")

                # Verify None handling
                if price > 0:
                    print("           âœ“ Valid data received")
                else:
                    print("           âš  No price data (possibly market closed)")

            print("\nâœ… PASS: Quote endpoint working correctly")
            return True
        else:
            print(f"âŒ FAIL: HTTP {response.status_code}")
            print(f"Response: {response.text}")
            return False

    except requests.exceptions.ConnectionError:
        print("âŒ FAIL: Cannot connect to backend")
        print("Please ensure backend is running on http://localhost:8000")
        return False
    except Exception as e:
        print(f"âŒ FAIL: {str(e)}")
        return False
...
def test_screener_endpoint():
    ...
    for screener_type in screener_types:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"  âœ“ Returned {len(results)} stocks")
    ...
    print("\nâœ… PASS: Screener endpoint working correctly")
    return True
    ...
    for i, quote in enumerate(test_cases, 1):
        ...
        try:
            if change >= 0:
                ...
                delta_color = "inverse"
            print(f"       âœ“ Comparison works, delta_color={delta_color}")
        except TypeError as e:
            print(f"       âŒ TypeError: {e}")
            return False

    print("\nâœ… PASS: None handling working correctly")
    return True
