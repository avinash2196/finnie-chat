...
def categorize_answer_source(rag_results, answer):
    ...
    if not rag_results:
        return {
            ...
            "confidence": 0.0,
            "sources": [],
            "warning": "âš ï¸ No RAG documents found. Answer may be hallucinated.",
    ...
    if avg_similarity > 0.5 and overlap > 0.25:
        ...
        confidence = 0.90
        warning = "âœ… High confidence - Answer grounded in trusted RAG sources"
    elif avg_similarity > 0.35 and overlap > 0.15:
        # Medium match: some similarity and reasonable overlap
        category = "RAG_INFORMED"
        confidence = 0.70
        warning = "ðŸŸ¡ Medium confidence - Answer based on RAG but with LLM interpretation"
        ...
        confidence = 0.50
        warning = "ðŸŸ¡ Low-Medium confidence - Partial RAG match, some LLM generation"
    else:
        # No meaningful match
        category = "LLM_GENERATED"
        confidence = 0.3
        warning = "âš ï¸ Low confidence - Answer may not be grounded in RAG sources"
...
def get_recommendation(category):
    ...
    recommendations = {
        "RAG_GROUNDED": "âœ… Trust this answer - it's from verified financial knowledge",
        "RAG_INFORMED": "ðŸŸ¡ Answer is based on trusted sources but includes interpretation. Verify for important decisions.",
        "RAG_PARTIALLY_MATCHED": "ðŸŸ¡ Answer is partially grounded in trusted sources. Consider additional research.",
        "LLM_GENERATED": "âš ï¸ Answer is not from verified sources. Independently verify before using for decisions."
    }
